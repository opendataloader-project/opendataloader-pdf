...
export function registerCliOptions(program: Command): void {
  ...
  program.option('--hybrid-hancom-ai-layout-page-chunk <value>', 'Pages per layout request. Requires --hybrid=hancom-ai. The backend rejects requests past roughly 30 pages, so longer documents are sliced; 0 disables slicing and sends every page in one request. Default: 20');
