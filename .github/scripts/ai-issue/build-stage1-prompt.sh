...
3. Is this UNCLEAR or MISSING REQUIRED INFO? → "needs-info"
   - Missing reproduction steps, environment, or details needed to act on it
   - **IMPORTANT: For bug reports (exceptions, errors, parsing issues, etc.), if no sample PDF/document is provided or linked that can reproduce the issue, ALWAYS mark as "needs-info" and request the document**
...
Respond with JSON only:
{
  ...
  "questions": ["question1", ...] // only if decision is "needs-info", should include request for sample document if applicable
}
PROMPT_EOF
