...
| `content_safety_off`      | `str \| list[str]`   | -            | Disable content safety filters. Values: all, hidden-text, off-page, tiny, hidden-ocg, sensitive-data                               |
...
| `parallel`                | `str`                | `"1"`        | Number of files to process in parallel (separate process per file). Default: 1 (sequential)                                        |
| `min_pages_parallel`      | `str`                | `"8"`        | Minimum page count to enable parallel page processing within a document. Default: 8                                                |
