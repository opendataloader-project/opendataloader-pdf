...
Reading order is enabled by default. Consider disabling (`--reading-order off`) for:

| Document Type | Recommendation |
|---------------|----------------|
| Single-column documents | Default works fine, but `off` is slightly faster |
| Forms, tables | May want `off` if layout detection interferes |
| Pre-sorted PDFs | Use `off` if PDF already has correct order |
