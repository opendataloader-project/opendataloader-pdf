...
| ---------------- | ------------------------------------------------------- | ----------- |
| `all`            | Enables every safety filter (default).                  | ✅          |
...
When `sensitive-data` is enabled, extracted emails/URLs may be replaced with placeholders (for example, `email@example.com` or `https://example.com`) in Markdown output.

To preserve original links and email addresses for trusted documents, disable only that filter:
...
opendataloader_pdf.convert(
    ...
    content_safety_off=["sensitive-data"],
)
```
