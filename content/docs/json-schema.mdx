...
The `source` field is present only when the image is successfully extracted to a file. It will be absent when:

- **`--image-output embedded`**: The `data` field (Base64) is used instead of `source`.
- **`--image-output off`**: Image extraction is disabled entirely.
- **Image rendering failed**: The image is detected in the PDF (bbox is present) but could not be rendered. This can happen with certain PDF image encodings or when the page cannot be rendered. A `WARNING` is logged in this case — check the tool output for `"Image could not be rendered and will be skipped"`.
